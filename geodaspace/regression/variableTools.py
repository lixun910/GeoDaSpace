...
class VarSearchCtrl(wx.SearchCtrl):
    def __init__(self, parent=None, id=-1, value="", pos=wx.DefaultPosition,
                 size=wx.DefaultSize, style=0, doSearch=None):
...
class vVariableSelector(wx.MiniFrame):
    ...
    def Search(self, text, inclusive):
        if inclusive:
            self.populate(
                [v for v in self.values if text.lower() in v.lower()])
        else:
            if text:
                self.populate(
                    [v for v in self.values if text.lower() not in v.lower()])
            else:
                self.populate(None)
    ...
    def getSelected(self):
        ...
        items = []
        while True:
            idx = self.sourcelist.GetNextItem(
                idx, state=wx.LIST_STATE_SELECTED)
            if idx == -1:
                break
            else:
...
class ddtApp(wx.App):
    ...
    def OnInit(self):
        values = [item + '_%d' % (random.randint(80, 99))
                  for item in "abcdefghijklmnopqrstuvwxyz" * 2]
...
if __name__ == '__main__':
    dataFile = '/Users/charlie/Documents/repos/inclusio/\
            trunk/inclusio/Data/Shapes/usa.dbf'
    app = ddtApp()
    app.MainLoop()
